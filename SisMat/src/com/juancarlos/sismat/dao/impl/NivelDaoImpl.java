...
public class NivelDaoImpl extends HibernateDaoSupport implements NivelDao {
	...
	public List<Nivel> listaNivel(String codigoColegio, String nivel,
			String estado) {
		...
		try {
			if (nivel.length() == 0 && estado.length() == 0) {
				sql = "from Nivel where codigoColegio = '"
						+ codigoColegio.trim() + "'";
			...
			if (nivel.length() != 0 && estado.length() != 0) {
				sql = "from Nivel where codigoColegio = '"
						+ codigoColegio.trim() + "' AND nivel='" + nivel
						+ "' AND estado='" + estado + "'";
